...
function test_generator_patterns()
    ...
    @testset "Generator Pattern Comparisons" begin
        ...
        function hft_manual_loops()
            decoder = decode(TensorMessage, msg_data)
            matrix = SpidersMessageCodecs.decode(decoder)
        ...
        function hft_iterators()
            ...
            # Create row iterator
            rows = (view(matrix, i, :) for i in 1:n_rows)
            
            # Map to structured data
            trades = ((row[1], row[2], row[3]) for row in rows)  # (bid, ask, volume)
            
            # Calculate all metrics in one pass using mapreduce on trades
            metrics = mapreduce(trades; init=(0.0, 0.0, 0.0, 1)) do ((bid, ask, volume),), (ts, tv, mv, mi)
                spread = ask - bid
                new_ts = ts + spread
                new_tv = tv + volume
                if volume > mv
                    (new_ts, new_tv, volume, mi + 1)  # Increment counter for index
                else
                    (new_ts, new_tv, mv, mi)
                end
            end
            
            total_spread, total_volume, max_volume, max_idx_counter = metrics
            avg_spread = total_spread / n_rows
            
            # Find actual max volume index (need separate pass for this)
            max_volume_idx = argmax(matrix[i, 3] for i in 1:n_rows)
            
            # Count profitable spreads
            profitable_count = count(((bid, ask, volume),) -> (ask - bid) > avg_spread * 1.1, trades)
        ...
        for (name, allocs) in allocations
            if name == "Manual Loops"
                @test allocs == 0  # Manual loops should achieve zero allocation
            elseif name == "Generators"
                @test allocs <= 100  # Generator pattern should have minimal allocation
            elseif name == "Ntuple Hybrid"
                @test allocs <= 100  # Ntuple hybrid should have minimal allocation
            elseif name == "Functional"
                @test allocs <= 200  # Functional pattern should have reasonable allocation
            elseif name == "Iterators"
                @test allocs <= 200  # Iterator pattern should have reasonable allocation
            end
        end
...
function test_advanced_generator_patterns()
    ...
    @testset "Advanced Generator Patterns" begin
        ...
        function rolling_window_stats(window_size=3)
            ...
            n_rows = size(matrix, 1)
            values = (matrix[i, 2] for i in 1:n_rows)  # Value column
            
            # Rolling windows using generators (lazy evaluation)
            windows = (collect(Iterators.take(Iterators.drop(values, i-1), window_size)) 
                      for i in 1:(n_rows-window_size+1))
            
            # Calculate rolling statistics
            rolling_means = (sum(window) / length(window) for window in windows)
            rolling_stds = (sqrt(sum((x - mean(window))^2 for x in window) / length(window)) 
                           for window in windows for mean in (sum(window) / length(window),))
            
            # Collect results (this will allocate, but generation is lazy)
            means = collect(rolling_means)
            stds = collect(rolling_stds)
